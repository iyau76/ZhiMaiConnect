...
| 4. 回忆与行动 | 关系图、人物卡、日历、提醒、“这事该拜托谁”和“问一问”           | 从已批准事实计算关系投影、路径和建议               |
...
录入 Agent、推荐 Agent 和“问一问”共用工具注册表、预算、运行日志和错误恢复。典型查询会先浏览人物索引，再读取命中的人物、关系或事件；档案规模扩大后，模型上下文仍只包含当前问题需要的部分。
...
3. 在 AI 助理中勾选“带上我的人物库”，查询某个人，或要求修改一条关系；
4. 批准变更后再次提问，再用“整批撤销”恢复上一版本。
...
| 位置                                     | 职责                                          |
| ---------------------------------------- | --------------------------------------------- |
| `src/components/`                        | 录入、人物关系、提醒、日历、AI 助理与设置界面 |
