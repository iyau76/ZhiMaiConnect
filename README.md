...
- **人物与关系**：人物卡、关系依据和交互式关系图；纯亲属视图自动切换为按世代排列的家族树，关系网支持圈层布局、Louvain 拓扑社区、不分组布局、一跳/两跳聚焦、全屏查看及单边常显/自动/常隐。
- **事件与时间**：人物关联事件、生日、节日、待办与提醒统一进入时间视图；事件可以只精确到月或年，日历同格显示公历和农历，也能保存“去年夏天”这类模糊日期。
...
终端会显示开发服务器的实际地址。没有配置云端模型密钥时，人物档案、关系图、日历、提醒、备份和本地算法仍可使用；AI 功能可以改用本机的 OpenAI 兼容推理服务。

## 模型与环境变量

知脉 Connect 支持两类模型来源：云端 OpenAI 兼容接口与 Gemini 兼容接口走受限同源代理，本机或局域网的 OpenAI 兼容推理服务由设备直连。
...
| 本机 OpenAI 兼容服务（Ollama、LM Studio 等）                    | 模型设置中填写 `http://localhost:11434/v1` 一类地址，可不填 Key | 设备直连本机或局域网，数据不出本机  |

Windows / Android 安装版通过原生网络直接访问配置的模型端点，无需网站代理，安装包不包含 API Key。Android 当前使用 HTTPS 接口，手机的 `localhost` 指手机自身。
...
本机推理服务需要允许当前网页来源访问（例如 Ollama 的 `OLLAMA_ORIGINS`）。只为可信的本机环境放行，不要把本地模型服务直接开放到公网。
...
- [复杂人物关系综合测试集](doc/quality/复杂人物关系综合测试集.md)
