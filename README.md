...
首次打开应用后，进入“设置”页，在“竞赛演示数据”区域点击“一键载入/重置合成数据”，即可创建一套可反复重置的虚构关系库：

- 50 位合成人物，覆盖家人、亲戚、大学同学、科研、摄影社和校友社群 6 个圈层
- 80 条类型化关系，包括亲属、同学、前室友、项目协作、汇报关系和跨圈联系路径
- 25 条共同事件和 3 条提醒，覆盖精确日期、月/年级模糊日期及时间范围
- 两位同名人物、历史昵称和 1 条明确标注的低置信度待核验关系，用于演示消歧与确认流程

载入后可直接体验关系图的一跳/两跳聚焦、圈层与拓扑社区布局、“这事该拜托谁”、日历、提醒和 AI 助理。再次点击同一按钮会按稳定 ID 覆盖重置这套合成数据；“只清除合成数据”不会删除用户自己录入的资料。

演示数据只写入当前浏览器的 IndexedDB。换浏览器、换设备或清理站点数据后需要重新载入，不会由仓库或 Cloudflare 自动同步给其他访问者。所有合成邮箱均使用 `example.invalid`，不对应真实个人。
