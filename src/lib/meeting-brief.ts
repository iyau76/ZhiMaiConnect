...
import { formatFuzzy } from "./fuzzy-date";
...
function buildContent(input: MeetingBriefInput, person: PersonRecord) {
  const own = personRef(person);
  const profile: MeetingBriefLine[] = [];
  const personProfile = person.profile;
  if (personProfile?.relation?.trim())
  ...
  const recentEvents = input.events
    .filter((event) => relatedTo(person.id, event.personIds))
    .sort(
      (left, right) =>
    ...
    .map((event) =>
      line(
        `${formatFuzzy(event)} · ${event.title}${event.detail?.trim() ? `：${event.detail.trim()}` : ""}`,
        eventRef(event),
      ),
    );
  ...
  const openItems = [...reminders, ...tasks]
  ...
  if (recentEvents[0]) {
    talkingPoints.push(
      line(
        `可以接着上次的“${recentEvents[0].text.split(" · ").slice(1).join(" · ")}”聊聊。`,
        ...recentEvents[0].sources,
      ),
    );
  }

  const gaps: string[] = [];
  if (!personProfile?.relation?.trim()) gaps.push("还没记录和我的关系");
  ...
  if (!recentEvents.length) gaps.push("还没有共同事件");
...
export function buildMeetingBrief(
  ...
  const content = buildContent(input, person);
  const allLines = [
    ...content.profile,
