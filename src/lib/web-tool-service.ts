...
export function createWebTools(readFixedUpstream: WebToolReader) {
  ...
  async function weather(location: string, request: Request) {
    ...
    const forecast = parseJson<{
      ...
      await readFixedUpstream(forecastUrl.toString(), request, "天气服务", 256 * 1024),
      "天气服务",
    );
