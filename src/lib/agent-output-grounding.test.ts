...
describe("assistant archive grounding", () => {
  ...
  it("accepts a model-composed relation hint but renders a canonical local field", () => {
    ...
    expect(result.ok).toBe(true);
    expect(result.evidenceText).toContain("前同事");
  });
  ...
  it("rejects person facts smuggled into the free analysis channel", () => {
    const result = validateAssistantArchiveGrounding({
      question: "何澜是做什么的？",
    ...
    expect(result.error).toContain("自由分析区不得陈述");
  });
  ...
  it("rejects pronoun and omitted-subject facts in archive commentary", () => {
