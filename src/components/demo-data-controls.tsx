...
export function DemoDataControls() {
  ...
  const load = async () => {
    ...
    try {
      ...
      await refresh();
      toast.success(`已载入 ${result.people} 位合成人物、${result.relations} 条关系`);
    } catch (error) {
  ...
  const clear = async () => {
    if (!window.confirm("只会删除带“合成演示数据”标识的记录，不影响你自己录入的资料。继续？")) {
      return;
    }
    setBusy(true);
    try {
    ...
    <section className="space-y-3 border-t border-border pt-5">
      <div>
        ...
        <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">
          一键载入 50 位虚构人物、80 条关系及配套事件。所有邮箱使用
          example.invalid，界面和来源均标注为合成数据，不对应真实个人。
        </p>
      </div>
      <p className="text-xs">
        当前已载入：{status.people} 人 · {status.relations} 条关系
      </p>
