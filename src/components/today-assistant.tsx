...
import { type ChatTurn, type ProviderPreset } from "@/lib/vision-providers";
...
/** 「今天」页底部的问一问：可以带上本机资料问人际问题，回答附可核对的档案依据。 */
export function TodayAssistant({
  ...
  const resumeAssistant = async () => {
    ...
    const resumePreset = preset.id === resumeRequest.presetId ? preset : undefined;
    if (!resumePreset) {
      toast.error("原模型配置已删除或已切换；请重新发送这个问题");
      return;
    }
    busyRef.current = true;
    setBusy(true);
      ...
      >
        ...
        <div ref={logRef} className="mt-3 max-h-72 space-y-2 overflow-y-auto">
          {turns.map((turn, index) => (
            <div
              key={index}
              data-assistant-turn={turn.role}
              className={cn(
                "rounded-lg px-3 py-2 text-sm",
                turn.role === "user"
                  ? "bg-accent/60 whitespace-pre-wrap"
                  : "bg-muted/50 [&_p]:text-foreground",
              )}
            >
              {turn.text ? (
                turn.role === "user" ? (
                  ...
                  <MarkdownView text={turn.text} className="text-sm" />
                )
              ) : (
                "…"
              )}
