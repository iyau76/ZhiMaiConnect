...
export function IntakePanel({
  ...
  const relationReviewCount = (draft?.relations ?? []).filter(
    (relation) =>
  ...
  const relationReviewAttention = relationReviewCount > 0;
      ...
      {draft && (
        ...
        >
          ...
          {relationReviewAttention && (
            <section
              role="alert"
              data-testid="intake-relation-review-alert"
              className="flex items-start gap-2.5 rounded-xl border border-amber-500/50 bg-amber-500/10 px-3 py-2.5 text-xs"
            >
              <TriangleAlert
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">
                  {t("下方「新关系」已自动展开。逐条核对来源，接受、修改或拒绝后再确认入库。")}
                </p>
              </div>
              <span className="ml-auto shrink-0 rounded-full border border-amber-500/40 px-2 py-0.5 text-[10px] text-amber-800 dark:text-amber-200">
                {relationReviewCount} {t("条待核验")}
              </span>
            </section>
          )}
          ...
          <ReviewFold
            ...
            attention={relationReviewAttention}
            summary={relationReviewAttention ? `${relationReviewCount} 条待核验` : undefined}
          >
            {(draft.relations ?? []).length > 0 && (
